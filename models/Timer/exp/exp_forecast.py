...
class Exp_Forecast(Exp_Basic):
    ...
    def finetune(self, setting):
        ...
        for epoch in range(self.args.finetune_epochs):
            ...
            loss_val = torch.tensor(0., device="cpu")
            count = torch.tensor(0., device="cpu")
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(finetune_loader):
                ...
                if self.args.output_attention:
                 ...
                 # 在计算损失前添加调试代码
                # print(f"Model outputs shape: {outputs.shape}")  # 应为 [batch_size, ?, feature_dim]
                # print(f"Batch_y shape: {batch_y.shape}")       # 应为 [batch_size, seq_len, feature_dim]
                # print(f"Pred_len: {self.args.pred_len}")       # 确认参数值
    ...
    def test(self, setting, test=0):
        ...
        if self.args.output_len_list is not None:
            for i in range(len(preds_list)):
                ...
                # 保存到文本文件（追加模式）
                txt_path = os.path.join(folder_path, 'result.txt')
